...
describe('Stripe checkout webhook handler', () => {
  beforeEach(() => {
    ...
    mockDb.stripeEvent.create.mockResolvedValue({} as never);
    mockDb.stripeEvent.update.mockResolvedValue({} as never);
  ...
  it('dispatches checkout.session.completed to the paid checkout confirmation service', async () => {
    ...
    expect(mockDb.stripeEvent.update).toHaveBeenCalledWith({
      where: { stripeEventId: 'evt_checkout_completed' },
      data: { status: 'PROCESSED', errorMessage: null },
    });
  });

  it('skips duplicate checkout events before dispatching confirmation', async () => {
    ...
    mockDb.stripeEvent.create.mockRejectedValue(duplicateError);
