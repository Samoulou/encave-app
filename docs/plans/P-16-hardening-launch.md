...
> **Statut** : en cours · **Branches** : `claude/p-16-ci-tests` (PR-1) · `claude/p-16-auth-hardening` (PR-2) · `claude/p-16-launch-hardening` (PR-3) · `claude/p-16-perf-should` (PR-4) · **PR** : #
...
**PR-1 `claude/p-16-ci-tests`** : ① ci.yml triggers dev + concurrency + job db-invariants (postgres service) ② `tests/db/gift-redemption-concurrency.test.ts` (FOR UPDATE réel, modèle booking-hold-concurrency) ③ k6 éphémère CI (`tests/k6/`, extraction Next-Action ID, assert SQL post-run) + `weekly-gates.yml` ④ infra e2e : webhook Stripe signé simulé (`generateTestHeaderString`), `.env.test.example`, seed étendu (flags ON, admin, bookings) ⑤ 6 specs `tests/e2e/flows/` + 5 POMs ⑥ LHCI 2 étages (PR warn-only build local / gate ≥95 staging) + `budgets.json` ⑦ `@axe-core/playwright` + specs a11y booking/cadeaux ⑧ L-214 : Dialog Radix lightbox, aria-pressed pills/chips, aria-live stepper, contrastes gold/cream.

**PR-2 `claude/p-16-auth-hardening`** (`src/server/better-auth.ts`) : G-1 hook after custom OTP/social miroir du plugin twoFactor (+ défense admin/layout ; fallback : deny Google/OTP pour ADMIN) — **validation preview login Google réel** ; G-2 deny changeEmail si non vérifié + notif ancienne adresse ; G-3 cap expiresAt au refresh (`session.update.before`).

**PR-3 `claude/p-16-launch-hardening`** : A.1 corrélation `transfer.metadata.bookingId` dans `payouts.queries.ts::getPayoutDetail` (garde stripeAccountId, DTO `kind:'gift'`, badge UI) ; A.3 ADR-0003 + reversal annulation (refund carte limité à `cardCents`, restauration bon post-confirmation, `transfers.createReversal` idempotent) ; E health probes réels (DB/Redis/Stripe, `?deep=1` lag StripeEvent + jobs FAILED, 200/503) + Sentry capture money-path + `withCronMonitor` sur les 9 crons ; H CGV/privacy/mentions + sitemap/robots + `docs/runbooks/` ×5 + checklist nLPD ; I flag env `COMING_SOON` dans le middleware + redirects www→apex + alignement `.env.example`.
