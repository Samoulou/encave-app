...
**Contrainte plan Vercel (découverte au deploy de la PR)** : le plan Hobby rejette toute expression cron sub-quotidienne (« Hobby accounts are limited to daily cron jobs », lien d'échec du deploy). Le drain horaire devient **8 entrées quotidiennes espacées de 3 h** (`?slot=N`, paths distincts) → « prochain run » ≤ 3 h au lieu de ≤ 1 h ; le rappel 21 h garde ses 2 entrées (query params) + garde Zurich, précision ±59 min du plan Hobby absorbée par la garde. **Upgrade Pro (prévu pour le launch) = re-simplifier en un seul `0 * * * *`.**
