    ...
    return (
      <div className={cn('flex h-full w-full', className)}>
        ...
        >
          {isUploading ? (
            <>
              <div className="mb-3 h-12 w-12 animate-spin rounded-full border-4 border-burgundy-600 border-t-transparent" aria-hidden="true" />
              <p className="text-base font-medium text-burgundy-700" aria-live="polite">Uploading...</p>
            </>
          ) : (
            <>
              <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-burgundy-100">
                <Upload className="h-8 w-8 text-burgundy-600" aria-hidden="true" />
              </div>
        ...
        {error && <p className="absolute bottom-4 left-4 text-sm text-red-600" role="alert">{error}</p>}
      </div>
    );
  }
    ...
    return (
      <div className={cn('flex h-full w-full', className)}>
        ...
        >
          {isUploading ? (
            <>
              <div className="h-8 w-8 animate-spin rounded-full border-4 border-burgundy-600 border-t-transparent" aria-hidden="true" />
              <p className="mt-3 text-sm font-medium text-burgundy-600" aria-live="polite">Uploading...</p>
            </>
          ) : (
            <>
              <div className="mb-2 flex h-12 w-12 items-center justify-center rounded-xl bg-stone-100 transition-colors group-hover:bg-burgundy-100">
                <Plus className="h-6 w-6 text-slate-400 transition-colors group-hover:text-burgundy-600" aria-hidden="true" />
              </div>
              <p className="text-sm font-medium text-slate-500">Add photo</p>
            </>
          )}
        </button>
        {error && <p className="absolute bottom-2 left-2 right-2 text-center text-xs text-red-600" role="alert">{error}</p>}
      </div>
    );
  }
      ...
      {displayUrl ? (
            ...
            {isUploading && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/50">
                <div className="flex flex-col items-center">
                  <div className="h-10 w-10 animate-spin rounded-full border-4 border-white border-t-transparent" aria-hidden="true" />
                  <p className="mt-3 text-sm font-medium text-white" aria-live="polite">Uploading...</p>
                </div>
              </div>
            )}
          </div>
        ...
        >
          <div className="text-center">
            {isUploading ? (
              <>
                <div className="mx-auto mb-3 h-10 w-10 animate-spin rounded-full border-4 border-burgundy-600 border-t-transparent" aria-hidden="true" />
                <p className="text-sm font-medium text-burgundy-700" aria-live="polite">Uploading...</p>
              </>
            ) : (
              <>
