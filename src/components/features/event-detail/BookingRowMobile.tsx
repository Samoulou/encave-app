  ...
  return (
    ...
    >
      ...
      <dl className="mt-3 grid grid-cols-2 gap-3 text-xs">
        <div>
          <dt className="text-slate-400">{t('columns.partySize')}</dt>
          <dd className="text-slate-700">
            {t('partySize.inline', { count: booking.guestCount })}
          </dd>
        </div>
        <div>
          <dt className="text-slate-400">{t('columns.reference')}</dt>
          <dd className="font-mono text-slate-700">{booking.reference}</dd>
        </div>
        <div>
          <dt className="text-slate-400">{t('columns.status')}</dt>
          <dd className="mt-0.5">
            <BookingStatusBadge status={booking.status} />
          </dd>
        </div>
        <div>
          <dt className="text-slate-400">{t('columns.checkIn')}</dt>
          <dd className="text-slate-700">
            {booking.checkedInAt
              ? t('checkIn.at', {
